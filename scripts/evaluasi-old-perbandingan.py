...
Alur:
  ...
  5. Bandingkan IF-only vs Cascade vs Random Forest
...
from analyzer import (
    ...
    detect_anomalies,
    MODEL_FEATURE_COLS,
    feature_engineering_request_level,
    detect_anomalies_request_level,
...
def filter_normal_entries(
    entries, rps_threshold=3.0, error_threshold=0.4, min_requests=5
):
    """Filter entri normal dengan membuang IP yang jelas anomali.

    IP dengan ciri jelas anomali (high rps, high error rate) dihapus
    dari baseline normal agar evaluasi tidak tercemar false positive.

    Parameter dari analisis distribusi traffic normal smartkelurahan.
    """
    if not entries:
        return entries
    ...
    # Hitung statistik per-IP
    ip_stats = {}
    for ip, group in df.groupby("ip"):
        rps = len(group) / max(
            (group["timestamp"].max() - group["timestamp"].min()).total_seconds(), 1
        )
        err_rate = (group["status"] >= 400).mean()
        ip_stats[ip] = {"rps": rps, "err_rate": err_rate, "count": len(group)}

    # Tentukan IP yang akan difilter
    filtered_ips = set()
    for ip, s in ip_stats.items():
        reasons = []
        if s["rps"] > rps_threshold:
            reasons.append(f"rps={s['rps']:.2f}")
        if s["err_rate"] > error_threshold and s["count"] >= min_requests:
            reasons.append(f"err={s['err_rate']:.2%}")
        if reasons:
            filtered_ips.add(ip)
    ...
    logger.info(
        f"  Filter baseline: {len(filtered_ips)} IP dihapus "
        f"(rps>{rps_threshold} atau err>{error_threshold:.0%}), "
        f"{len(result)}/{len(entries)} entries tersisa"
    )
    if filtered_ips:
        for ip in sorted(filtered_ips)[:10]:
            s = ip_stats[ip]
            logger.info(
                f"    ✗ {ip:<20} req={s['count']:>4} rps={s['rps']:.2f} err={s['err_rate']:.2%}"
            )
        if len(filtered_ips) > 10:
            logger.info(f"    ... dan {len(filtered_ips)-10} IP lainnya")
...
if __name__ == "__main__":
    ...
    # # ---------- 5. PER-REQUEST IF (seperti paper Chua et al. 2024) ----------
    # logger.info(f"\n{'='*60}")
    # logger.info("BAGIAN 5: PER-REQUEST ISOLATION FOREST")
    # logger.info("(Setiap baris log = 1 sampel — 16 fitur request-level)")
    # logger.info("(Parameter: contamination=0.03, max_samples=0.25, n_estimators=200)")
    # logger.info(f"{'='*60}")

    # # Label per-request: setiap request dari IP anomali = anomaly
    # logger.info(f"\n  Melabel request dari IP anomali...")
    # y_true_req = pd.Series([e["ip"] in attack_ips for e in all_entries], dtype=int)
    # n_anom_req = int(y_true_req.sum())
    # n_norm_req = len(y_true_req) - n_anom_req
    # logger.info(
    #     f"  Total request: {len(all_entries)} (normal={n_norm_req}, anomali={n_anom_req})"
    # )

    # # Feature engineering per-request
    # logger.info(
    #     f"\n  Feature engineering per-request ({len(REQUEST_FEATURE_COLS)} fitur)..."
    # )
    # req_features = feature_engineering_request_level(all_entries)

    # # Tuning contamination (per-request)
    # logger.info(f"\n  Tuning contamination:")
    # logger.info(
    #     f"  {'Contam':>8} {'TP':>6} {'FP':>6} {'FN':>6} {'Prec':>8} {'Recall':>8} {'F1':>8}"
    # )
    # logger.info(f"  {'-'*54}")

    # best_req = None
    # for c in [0.01, 0.02, 0.03, 0.05, 0.08, 0.10]:
    #     r = detect_anomalies_request_level(req_features, contamination=c)
    #     yp = (r["req_anomaly"].values == -1).astype(int)
    #     m = hitung(y_true_req.values, yp)
    #     m["c"] = c
    #     logger.info(
    #         f"  {c:>8.2f} {m['tp']:>6} {m['fp']:>6} {m['fn']:>6} {m['prec']:>8.4f} {m['rec']:>8.4f} {m['f1']:>8.4f}"
    #     )
    #     if best_req is None or m["f1"] > best_req["f1"]:
    #         best_req = m

    # logger.info(
    #     f"\n  → IF per-request terbaik: contamination={best_req['c']:.2f}, F1={best_req['f1']:.4f}"
    # )

    # r_req = detect_anomalies_request_level(req_features, contamination=best_req["c"])
    # yp_req = (r_req["req_anomaly"].values == -1).astype(int)
    # met_req = hitung(y_true_req.values, yp_req)
    # logger.info(
    #     f"\n  Confusion Matrix (IF per-request, contamination={best_req['c']}):"
    # )
    # logger.info(f"  {'':>20} {'Pred Normal':>12} {'Pred Anomali':>12}")
    # logger.info(f"  {'Aktual Normal' :>20} {met_req['tn']:>12} {met_req['fp']:>12}")
    # logger.info(f"  {'Aktual Anomali':>20} {met_req['fn']:>12} {met_req['tp']:>12}")
    # logger.info(f"\n  Precision: {met_req['prec']:.4f} ({met_req['prec']*100:.2f}%)")
    # logger.info(f"  Recall:    {met_req['rec']:.4f} ({met_req['rec']*100:.2f}%)")
    # logger.info(f"  F1-Score:  {met_req['f1']:.4f} ({met_req['f1']*100:.2f}%)")
    # logger.info(f"  Accuracy:  {met_req['acc']:.4f} ({met_req['acc']*100:.2f}%)")

    # # Bandingkan dengan paper
    # logger.info(f"\n  Perbandingan dengan Paper Chua et al. (2024):")
    # logger.info(f"  {'':<35} {'Hasil':>8} {'Paper':>8}")
    # logger.info(f"  {'-'*51}")
    # logger.info(f"  {'Accuracy':<35} {met_req['acc']*100:>7.1f}% {93:>7}%")
    # logger.info(f"  {'Precision':<35} {met_req['prec']*100:>7.1f}% {95:>7}%")
    # logger.info(f"  {'Recall':<35} {met_req['rec']*100:>7.1f}% {90:>7}%")
    # logger.info(f"  {'F1-Score':<35} {met_req['f1']*100:>7.1f}% {92:>7}%")

    # # ---------- 6. SUPERVISED ----------
    # logger.info(f"\n{'='*60}")
    # logger.info("BAGIAN 6: RANDOM FOREST (supervised) — PEMBANDING")
    # logger.info(f"{'='*60}")
    ...
    # rf.fit(X_t, y_t)
    # y_rf = rf.predict(X_v)
    # met_rf = hitung(y_v, y_rf)

    # logger.info(f"\n  Confusion Matrix:")
    # logger.info(f"  {'':>20} {'Pred Normal':>12} {'Pred Anomali':>12}")
    # logger.info(f"  {'Aktual Normal' :>20} {met_rf['tn']:>12} {met_rf['fp']:>12}")
    # logger.info(f"  {'Aktual Anomali':>20} {met_rf['fn']:>12} {met_rf['tp']:>12}")
    # logger.info(f"\n  Precision: {met_rf['prec']:.4f} ({met_rf['prec']*100:.2f}%)")
    # logger.info(f"  Recall:    {met_rf['rec']:.4f} ({met_rf['rec']*100:.2f}%)")
    # logger.info(f"  F1-Score:  {met_rf['f1']:.4f} ({met_rf['f1']*100:.2f}%)")
    ...
    # logger.info(
    #     f"  {'3. IF per-request (16 fitur)':<45} {met_req['prec']:>8.4f} {met_req['rec']:>8.4f} {met_req['f1']:>8.4f} {met_req['tp']:>4} {met_req['fp']:>4}"
    # )
    # logger.info(
    #     f"  {'4. Random Forest (supervised)':<45} {met_rf['prec']:>8.4f} {met_rf['rec']:>8.4f} {met_rf['f1']:>8.4f} {met_rf['tp']:>4} {met_rf['fp']:>4}"
    # )

    # if met_req["f1"] > 0:
    #     delta_vs_if = (
    #         (met_req["f1"] - met_if["f1"]) / met_if["f1"] * 100 if met_if["f1"] else 0
    #     )
    #     logger.info(
    #         f"\n  Perbandingan F1: IF-only (per-IP)={met_if['f1']:.4f} → IF per-request={met_req['f1']:.4f} ({delta_vs_if:+.1f}% vs per-IP)"
    #     )
    # logger.info(
    #     f"  Paper Chua et al. (2024): Accuracy=93%, Precision=95%, Recall=90%, F1=0.92"
    # )
    logger.info(f"\n  Konfigurasi optimal:")
    logger.info(f"    Contamination (IF-only)       : {best_if['c']}")
    # logger.info(f"    Contamination (per-request)   : {best_req['c']}")
    logger.info(f"    Max Samples (per-request)     : 0.25 (sesuai paper)")
    logger.info(
        f"    Fitur per-request             : {len(REQUEST_FEATURE_COLS)} ({', '.join(REQUEST_FEATURE_COLS)})"
    )
